...
def get_data(path, size ,train=True, small=True):
    ...
    if small:
        num_of_train_images = 211
        num_of_test_images = 114
    else:
        num_of_train_images = 21100
        num_of_test_images = 11400
    if train:
        X = np.zeros((num_of_train_images, size[0], size[1], 1), dtype=np.float32)
        y = np.zeros((num_of_train_images, size[0], size[1], 1), dtype=np.float32)
        list = train_text_file_list
    else:
        X = np.zeros((num_of_test_images, im_height, im_width, 1), dtype=np.float32)
...
X_train, y_train = get_data("https://storage.googleapis.com/uga-dsp/project2",(512,512) ,train=True, small=False)
